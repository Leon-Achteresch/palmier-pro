...
final class TimelineHeaderView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for (i, track) in editor.timeline.tracks.enumerated() {
            ...
            let recordX = keyframeX - iconSize - AppTheme.Spacing.xs

            let labelX = gripX + iconSize + AppTheme.Spacing.sm
            let labelRightX = track.type == .audio ? recordX : keyframeX
            let labelHeight = min(
            ...
            let labelRect = NSRect(
                x: labelX,
                y: y + (h - labelHeight) / 2,
                width: max(0, labelRightX - AppTheme.Spacing.xs - labelX),
                height: labelHeight
            )
