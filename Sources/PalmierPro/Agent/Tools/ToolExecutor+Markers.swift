...
private struct ManageMarkersInput: DecodableToolArgs {
    ...
    var hasPatch: Bool {
        name != nil || startFrame != nil || durationFrames != nil || color != nil
            || comment != nil || status != nil || kind != nil
    }
}
...
extension ToolExecutor {
    func manageMarkers(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        ...
        let kind: MarkerKind? = try input.kind.map {
            guard let kind = MarkerKind(rawValue: $0) else {
                throw ToolError("kind must be one of \(MarkerKind.allCases.map(\.rawValue).joined(separator: ", ")).")
            }
            return kind
        }
