...
"enum": ["volumeDb", "opacity", "rotation", "position", "scale", "crop", "speed", "blur"],
        ...
        AgentTool(
            name: .addCaptions,
            brief: "Transcribe the timeline's spoken audio and create caption clips on their own track — no targeting needed — or place an imported SRT/WebVTT file's cues via subtitleMediaRef. Returns a captionGroupId; restyle later with update_text. Uses cloud transcription only when the account has credits, otherwise local.",
        ...
        AgentTool(
            ...
            description: "Lists AI models with their capabilities (durations, aspect ratios, resolutions, draft mode, first/last frame support, reference support, voices/category for audio, and configurable settings for upscalers). Models with usesOwnApiKey:true run directly on the user's own provider key (OpenRouter for image/video, ElevenLabs for audio) and are billed there instead of in Palmier credits. Always call before generate_video, generate_transition, generate_image, generate_audio, or upscale_media so the model you pick actually supports the constraints you need. For transitions, pick a video model with supportsFirstFrame and supportsLastFrame. Returns { models, loaded } — if loaded=false the catalog hasn't synced yet (e.g. user not signed in); the models array may be empty even when models exist, so do not conclude no models are available. Retry after the user signs in.",
            inputSchema: objectSchema(
                properties: [
