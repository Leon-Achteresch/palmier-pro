...
struct AgentTool: @unchecked Sendable {
    ...
    init(name: ToolName, brief: String? = nil, description: String, inputSchema: [String: Any]) {
        self.name = name
        self.brief = brief ?? description
...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            name: .manageMarkers,
            brief: "Create, update, or delete one persistent timeline marker — review notes, todos, and chapter markers that feed the export chapter sidecar. Markers never change the cut; use stable markerId values from get_timeline.",
            description: "Creates, updates, or deletes one persistent timeline marker. A zero duration marks one frame; a positive duration is half-open. Status tracks the review workflow: open is awaiting work, review is ready for user approval, and resolved is accepted. Set review only after applying and verifying the requested edit. Set resolved only when the user explicitly approves or requests it. Chapter markers (kind 'chapter') drive a YouTube-style '<timestamp> <name>' sidecar written next to a video export — name them, and start one at frame 0 for a valid list.",
            inputSchema: objectSchema(
                properties: [
        ...
        AgentTool(
            name: .importMedia,
            brief: "Import external media into the library from an HTTPS url, local path (referenced in place), inline base64 bytes, or a generated solid-color matte. URL imports run in the background — poll get_media until generationStatus clears. Free; only mov/mp4/m4v, common audio, png/jpg/tiff/heic, and srt/vtt subtitles are accepted.",
        ...
        AgentTool(
            name: .manageTracks,
            brief: "Reorder, name, configure (mute, hide, sync-lock), or remove tracks in one undoable action; index 0 renders on top. Prefer stable trackId selectors from get_timeline. Removing a track deletes all its clips; multicam tracks can't be removed.",
        ...
        AgentTool(
            name: .swapClipMedia,
            brief: "Point an existing clip at a different library asset while keeping its edit, timing, framing, effects, and keyframes. For replacing a take or a placeholder with the final asset; relink_media is for reconnecting offline files instead. Same source type required.",
        ...
        AgentTool(
            ...
            brief: "Set static clip values on one or more clips: trims, duration, speed, volume, opacity, fades, edges, transform, crop, blend mode, audioMix, duckingRole. Not for multi-clip layouts (apply_layout), animation (set_keyframes), edge trims/slips (trim_clips), or text (update_text). Setting a static value clears that property's keyframes.",
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "duckingRole": [
                        "type": "string",
                        "enum": DuckingRole.allCases.map(\.rawValue),
                        "description": "Audio clips only. Overrides how auto-ducking sees this clip: 'dialog' makes it duck the beds, 'bed' makes it duck under speech, 'exempt' leaves it alone, 'auto' (default) classifies it from its own on-device speech detection. Set this when the automatic call is wrong — a music bed with vocals read as dialog, or narration on a music track. Ducking itself is switched on by mix_audio.",
                    ],
                    "crop": [
                        "type": "object",
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "tracks": [
                        "type": "object",
                        "description": "Animate several properties at once: {property: keyframe rows}, keys from volumeDb, opacity, rotation, position, scale, crop, speed, blur. An empty array clears that property's track. Mutually exclusive with property/keyframes. \(keyframeRowShapes)",
                        "additionalProperties": ["type": "array", "items": ["type": "array"]],
                    ],
                    "property": [
                        "type": "string",
"enum": ["volumeDb", "opacity", "rotation", "position", "scale", "crop", "speed", "blur"],
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "slots": [
                        ...
                        "items": objectSchema(
                            properties: [
                                "slot": ["type": "string", "description": "Slot name for the chosen layout. Slots per layout — full: main; side_by_side: left, right; top_bottom: top, bottom; pip_*: main, inset; grid_2x2/3x3/4x4: rNcN counted from the top-left (r1c1 is top-left, r2c2 a 3x3's center); main_sidebar: main, sidebar; three_up: left, center, right; three_stack: top, middle, bottom. Every slot of the layout must be filled."],
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "language": ["type": "string", "description": "Optional BCP-47 speech language. On-device models are language-specific and produce garbled output when the wrong language is used. Defaults to the system language."],
                ]
            )
        ),
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "language": ["type": "string", "description": "Optional BCP-47 speech language. Omit to reuse the previous get_transcript language."],
                ],
                required: []
            )
        ),
        AgentTool(
            name: .removeSilence,
            brief: "Remove dead air — quiet, speech-free sections — and ripple-close the gaps, in one undoable action. Whole timeline by default, or scope with clipIds; override minimumPauseSeconds/speechPaddingSeconds one-shot. Handles pauses; remove_words handles fillers, ripple_delete_ranges arbitrary spans. No transcript needed.",
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    "platform": [
                        "type": "string",
                        "enum": MixPlatformPreset.allCases.map(\.rawValue),
                        "description": "Delivery target (default youtube). youtube −14 LUFS program, podcast −16, film −23.",
                    ],
                    "dryRun": ["type": "boolean", "description": "true returns the classification and planned gains without touching the timeline and without an undo step."],
                    "ducking": ["type": "boolean", "description": "Turn auto-ducking on as part of the mix (default true). false levels the clips and leaves the ducking setting as it is."],
                    "duckDepthDb": [
                        "type": "number",
                        "minimum": DuckingLimits.depthDb.lowerBound,
                        "maximum": DuckingLimits.depthDb.upperBound,
                        "description": "How far beds drop under speech, in dB (default −12). Only valid while ducking is on.",
                    ],
                ]
            )
        ),
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: mergedProperties([
                    ...
                    "language": ["type": "string", "description": "BCP-47 speech language. On-device models are language-specific and produce garbled output when the wrong language is used. Defaults to the system language."],
        ...
        AgentTool(
            ...
            description: "Lists AI models with their capabilities (durations, aspect ratios, resolutions, draft mode, first/last frame support, reference support, voices/category for audio, and configurable settings for upscalers). Every model runs directly on the user's own provider key (OpenRouter or Google AI for image/video, ElevenLabs for audio) and is billed there. Always call before generate_video, generate_transition, generate_image, generate_audio, or upscale_media so the model you pick actually supports the constraints you need. For transitions, pick a video model with supportsFirstFrame and supportsLastFrame. Returns { models, loaded } — if loaded=false no provider key is configured yet, so the models array may be empty; ask the user to add an API key in Settings › Agent.",
            inputSchema: objectSchema(
                properties: [
        ...
        AgentTool(
            ...
            description: "Starts an async AI audio generation or transformation. Returns a placeholder asset ID immediately; the asset appears in get_media and becomes usable in add_clips once ready. TTS converts text into speech. Generative audio models create dialogue, music, or sound effects from a prompt, video, or supported image/audio references. Voice Cleanup isolates speech from background audio. Dubbing translates source speech while preserving speaker delivery; pass targetLanguage. For models whose inputs include audio or video, provide sourceMediaRef. Other results land in the media library for placement with add_clips. Use list_models with type='audio' to inspect inputs, category, voices, reference caps, and limits. Audio models run directly on the user's own ElevenLabs key and are billed by ElevenLabs. Costs real money and is not undoable.",
            inputSchema: objectSchema(
                properties: [
                    ...
                    "duration": ["type": "integer", "description": "Length in seconds. ElevenLabs Music: 3–600. Sonilo text-to-music: up to 600. Video-to-audio models default to the source length; pass a value to request a different output length. Voice Cleanup and Dubbing always preserve the source length. Ignored by TTS, MiniMax, and Lyria 3 Pro."],
                    ...
                    "videoSourceMediaRef": ["type": "string", "description": "Video-to-audio models only. Score this existing video asset."],
                    "folder": ["type": "string", "description": "Optional destination folder path, e.g. 'Hero shots/Takes'. Created if missing. Omit for the project root."],
                ],
                required: []
            )
    ...
    static var inAppAgent: [AgentTool] { all + [readSkill, manageSkills, describeTools] }
