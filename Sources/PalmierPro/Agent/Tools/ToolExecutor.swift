...
final class ToolExecutor {
    ...
    private let toolSessionID = UUID().uuidString
    ...
    func execute(
        ...
        let origin = ToolOrigin.Value(source: source, sessionID: sessionID ?? toolSessionID)
        return await ToolOrigin.$current.withValue(origin) {
    ...
    private func run(_ tool: ToolName, _ editor: EditorViewModel, _ args: [String: Any]) async throws -> ToolResult {
        ...
        case .applyMotion:      return try applyMotion(editor, args)
