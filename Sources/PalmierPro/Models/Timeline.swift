...
extension Timeline {
    ...
    mutating func rescaleMarkerFrames(by scale: Double) {
        ...
        for i in markers.indices {
            markers[i].rescaleFrames(by: scale)
        }
        sortMarkers()
    }
    ...
    private mutating func sortMarkers() {
        markers.sort { ($0.startFrame, $0.id) < ($1.startFrame, $1.id) }
    }
}
...
struct Track: Codable, Sendable, Equatable, Identifiable {
    ...
    private enum CodingKeys: String, CodingKey {
        case id, type, name, muted, hidden, syncLocked, clips, transitions, displayHeight
    }
}
